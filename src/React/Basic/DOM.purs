...
render :: JSX -> Node -> Effect Unit
render jsx node = render' jsx node (pure unit)

render' :: JSX -> Node -> Effect Unit -> Effect Unit
render' = runEffectFn3 render_

foreign import render_ :: EffectFn3 JSX Node (Effect Unit) Unit

hydrate :: JSX -> Node -> Effect Unit
hydrate jsx node = hydrate' jsx node (pure unit)

hydrate' :: JSX -> Node -> Effect Unit -> Effect Unit
hydrate' = runEffectFn3 hydrate_

foreign import hydrate_ :: EffectFn3 JSX Node (Effect Unit) Unit

unmountComponentAtNode :: Node -> Effect Boolean
unmountComponentAtNode = runEffectFn1 unmountComponentAtNode_

foreign import unmountComponentAtNode_ :: EffectFn1 Node Boolean
